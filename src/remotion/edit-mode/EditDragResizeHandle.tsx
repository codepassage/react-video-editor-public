...
export const EditDragResizeHandle: React.FC<{
  type: 'top-left' | 'top-right' | 'bottom-left' | 'bottom-right';
  setItem: (itemId: string, updater: (item: EditDragItem) => EditDragItem) => void;
  item: EditDragItem;
}> = ({ type, setItem, item }) => {
  ...
  const onPointerDown = useCallback((e: React.MouseEvent) => {
    ...
    const onPointerMove = (pointerMoveEvent: PointerEvent) => {
      ...
      setItem(item.id, (i) => {
        ...
        return {
          ...i,
          width: Math.max(20, Math.round(newWidth)), // 최소 크기 20px
          height: Math.max(20, Math.round(newHeight)),
          left: Math.min(item.left + item.width - 20, Math.round(newLeft)),
          top: Math.min(item.top + item.height - 20, Math.round(newTop)),
          isDragging: true,
        };
      });
    };
